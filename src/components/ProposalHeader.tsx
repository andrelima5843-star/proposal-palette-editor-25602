...
export const ProposalHeader = ({
  ...
  return <div className="bg-gradient-to-br from-secondary via-card to-background border-2 border-accent/30 p-8 md:p-12 rounded-2xl shadow-gold relative overflow-hidden">
      ...
      <div className="relative grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
        ...
        <div className="space-y-6">
          ...
          <p className="text-lg text-muted-foreground">
            ...
            {isEditing ? <Input value={companyName} onChange={e => setCompanyName(e.target.value)} className="inline-flex w-auto" /> : <span className="font-semibold text-accent" style={{
            textShadow: "0 0 10px hsl(45 100% 51% / 0.3)"
          }}>
                {companyName}
              </span>}{" "}
          ...
          <div className="flex flex-wrap gap-4">
            ...
            <div className="flex items-center gap-2 bg-card border border-accent/20 px-4 py-2 rounded-lg backdrop-blur-sm shadow-gold">
              ...
              <span className="text-foreground">​Móveis 
            </span>
            </div>
          </div>
        </div>
        ...
        transformStyle: "preserve-3d"
          ...
          <div className="flex items-center gap-4 mb-4">
            <div className="w-20 h-20 bg-gradient-to-br from-accent/20 to-primary/20 border-2 border-accent rounded-full flex items-center justify-center relative p-2 shadow-gold">
              {logoUrl ? <img src={logoUrl} alt="Logo" className="w-full h-full rounded-full object-cover" /> : <img src={novalarLogo} alt="Novalar Logo" className="w-full h-full object-contain" />}
              {isEditing && <label className="absolute -bottom-1 -right-1 bg-accent text-secondary p-1.5 rounded-full cursor-pointer hover:bg-accent/90 transition-colors shadow-gold">
            ...
            <div className="flex-1">
              {isEditing ? <>
                  <Input value={companyName} onChange={e => setCompanyName(e.target.value)} className="font-bold text-xl mb-2" />
                  <Input value={tagline} onChange={e => setTagline(e.target.value)} />
                </> : <>
                  <h2 className="font-bold text-2xl text-accent" style={{
                textShadow: "0 0 15px hsl(45 100% 51% / 0.4)"
              }}>
                    {companyName}
                  </h2>
          ...
          <div className="bg-gradient-to-r from-accent to-primary text-secondary p-4 rounded-lg shadow-gold border border-accent/50">
            <div className="flex items-center gap-2">
              <span>📍</span>
              {isEditing ? <Input value={location} onChange={e => setLocation(e.target.value)} className="bg-secondary/20 border-accent/30 text-foreground" /> : <span className="font-medium">{location}</span>}
            </div>
          </div>
        </div>
      </div>
